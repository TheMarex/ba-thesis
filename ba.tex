...
\subsection{Sloped ground}\label{sloped-ground}

\todo{actual-zmp-stabilized actual-zmp-unstabilized} \todo{image-series}

\section{Push recovery}\label{push-recovery-1}

The performance of the push recovery was evaluted with a simple
...
methode it is not reliable in the general case. This has multiple
reason, for one the fall detection is not very reliable. Another reason
is that the speed limits of the leg joints impose a maximum velocity for
the foot movement. So not only needs the target position to be
reachable, it also needs to be reachable in a specific amount of time.
